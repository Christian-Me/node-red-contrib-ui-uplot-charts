(function (global, factory) {
  ...
  class TimeTable {
     ...
     * @param {number} [offset=0] number of rows from end (default = 0)
     * @return {number} timestamp
     **/
    getLastTimestamp(offset = 0) { return this.content[0][this.content[0].length-1-offset] };
    /**
     * get timestamp of the table
     * @param {number} [offset=0] number of rows from beginning (default = 0)
     * @return {number} timestamp
     **/
    getTimestamp(offset = 0) { return this.content[0][offset] };
     /**
    ...
    remapRows(ids) {
      if (Array.isArray(ids) && ids.length>0) {
        ...
        var tempTable = [];
        tempTable[0]=this.content[0] || [];
        ids.forEach((id,index) => {
          if (!mappingFlag && index+1!==this.rowMap[id]) { mappingFlag = true;}        if (this.hasRow(id)) { // use existing row
            tempTable[index+1] = this.content[this.rowMap[id]] || [];
          } else { // create new row
            tempTable[index+1] = Array(this.content[0].length);
        ...
        this.content=tempTable;
        if (mappingFlag) {this.node?.log(`remappedRows to [${Object.keys(this.rowMap).toString()}]`);}
      }
      return this.content;
    }
    /**
    ...
    **/
    ...
    applyRules(rules) {
      ...
      if (rules === undefined || !Array.isArray(rules)) {
        this.node?.warn(`applyRules: rules are undefined! exiting.`);
        return -1;
      }
