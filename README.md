...
This node-red dashboard node is highly customizable inside the editor.
...
* customizable dynamic labels on time scales depending on visible period
* Zoom and pan features (mouse & touch)
* customizable persistent chart data
* data reduction on historic data
...
* title and legend sizes not calculated automatically resulting in scroll bars (set the height values for now)
...
[ ] more useful input formats  
[ ] support of non time based x-axes
...
Either use the Editor - Menu - Manage Palette - Install option, or run the following command in your Node-RED user directory (typically `~/.node-red`) after installing Node-RED-dashboard.
    ...
    npm i node-red-contrib-ui-uplot-chars
...
Data series are defined here. They will be auto added and configured as soon as a unknown `mgs.topic` arrives but can be edited or pre defined.

Data series can be sorted to have control over the draw order

|property | type | description
|--------:|:-----|------------
|topic|string| unique identifier for that data series. Will be read only as soon as one datapoint with that topic arrived.
...
|pattern|array| dash pattern as described [here](https://developer.mozilla.org/en-US/docs/Web/API/CanvasRenderingContext2D/setLineDash)
...
The X-axes is defined by an JSON object. Two pre defined configuration can be loaded (24h and 12h). [More information on how to configure the JSON can be found here.](https://github.com/leeoniya/uPlot/tree/master/docs#axis--grid-opts)

More axes can be defined:
...
Scales can be defined individually and used for data series and axes. In this way data with different units can be compared in an controlled way.
...
uPlot-charts database is a table. The database can be stored in the node, flow or global context. For the node context the data store can be selected. For flow and global context a unique name has to be provided.
...
| | period | period | period of time to sum up the data |
...
|**time precision**<td colspan=3>combine data rows if they fall into a reduced precise period. uPlot works by default in seconds. Fractional seconds are possible.|
...
uPlot-charts use a own library (timeTable) for handling data on the front and backend.

To avoid bottlenecks the data is written to file (if configured) currently only at a gracefully shutdown. A crash will result in a loss of data since last Node-Red start.

uPlot-charts use sparse arrays for data storage in order to achieve efficient memory use and performance. It is expected that data arrives with individual time stamps. This results in a new column for every datapoint. If multiple rows are present in a chart all the other rows would be a undefined value and add to the data consumption.

The fasted way to "clean" sparse arrays (get rid of empty columns) as I found out is cloning all data to a new array.

As JSON can't handle holes in arrays uPlot-charts uses more efficient CSV format for server client transfers and data storage especially for undefined values.

### plugins

uplot supports a kind of plugins to expand its capabilities.

uplot-charts can include these (currently) with some minor changes in the code base to provide the config data and initialization on the frontend.
